...
from simlane.iracing.types import Schedule
...
from simlane.sim.models import SessionType
...
class ScheduleProcessor:
    ...
    def process_season_schedule(
    ...
    ) -> tuple[int, int, int, list[str], int, int, int, int, list[str]]:
        """
        ...
        """
        logger.info(
            f"Processing schedule for season {season.name} with {len(season_data['schedules'])} weeks"
        )

        for schedule_data in season_data["schedules"]:
            try:
                self._process_week_schedule(season, schedule_data, season_data)
        ...
        return (
            self.events_created,
            self.events_updated,
            self.time_slots_created,
            self.weather_tasks,
            self.event_sessions_created,
            self.event_sessions_updated,
            self.event_classes_created,
            self.event_classes_updated,
            self.errors,
        )

    def _process_week_schedule(
        self, season: Season, schedule_data: Schedule, season_data: SeriesSeasons
    ) -> None:
        ...
        is_team_event = season_data.get("driver_changes", False)
        min_team_drivers = season_data.get("min_team_drivers", 1)
        max_team_drivers = season_data.get("max_team_drivers", 1)
        fair_share_pct = season_data.get("fair_share_pct", 25)
        must_use_diff_tire_types_in_race = season_data.get(
            "must_use_diff_tire_types_in_race", False
        )
        multiclass = season_data.get("multiclass", False)
        registration_open_minutes = season_data.get("registration_open_minutes", 30)
        ...
        required_compounds = {
            "must_use_diff_tire_types_in_race": must_use_diff_tire_types_in_race,
        }
        simulated_start_time = schedule_data.get("weather", {}).get(
            "simulated_start_time", None
        )
        additional_details = {
            "num_fast_tows": schedule_data.get("num_fast_tows", 0),
            "qualifier_must_start_race": schedule_data.get(
            ...
            "ignore_license_for_practice": schedule_data.get(
        ...
        self._process_car_restrictions(
            event, schedule_data.get("car_restrictions", []), fixed_setup
        )
        ...
        self._process_time_patterns(
            event, schedule_data.get("race_time_descriptors", [])
        )

        # Process weather
        if event.weather_forecast_url:
            self.weather_tasks.append(event.id)

    def _process_event_sessions(
        self, event: Event, schedule_data: Schedule, simulated_start_time: str
    ) -> None:
        ...
        race_time_limit = schedule_data.get("race_time_limit", None)
        race_lap_limit = schedule_data.get("race_lap_limit", None)
        simulated_time_multiplier = schedule_data.get("weather", {}).get(
            "simulated_time_multiplier", 1
        )
        simulated_time_offsets = schedule_data.get("weather", {}).get(
        ...
        if qual_attached:
            _event_session, created = EventSession.objects.update_or_create(
                ...
                defaults={
                    ...
                    "in_game_time": in_game_time
                    + timedelta(
                        minutes=simulated_time_offsets[0] * simulated_time_multiplier
                    ),
                },
            )
            if created:
        ...
        if race_time_limit or race_lap_limit:
            _event_session, created = EventSession.objects.update_or_create(
                ...
                defaults={
                    ...
                    "in_game_time": in_game_time
                    + timedelta(
                        minutes=simulated_time_offsets[1] * simulated_time_multiplier
                        if len(simulated_time_offsets) > 1
                        else simulated_time_offsets[0] * simulated_time_multiplier
                    ),
                },
            )
            if created:
    ...
    def _find_sim_layout(
        self, track_id: int, track_name: str, layout_name: str
    ) -> SimLayout | None:
    ...
    def _create_or_update_event(
        ...
        simulated_start_time = weather_data.get("simulated_start_time", None)
        ...
        if week_end_time < timezone.now():
            ...
            timezone.make_aware(start_date, timezone.get_default_timezone())
            < timezone.now()
            and week_end_time > timezone.now()
        ):
        ...
        event_defaults = {
            ...
            "enable_pitlane_collisions": schedule_data.get(
            ...
            "allowed_car_class_ids": car_class_ids,
            ...
            "registration_open_minutes": registration_open_minutes,
        }
    ...
    def _process_car_restrictions(
        self, event: Event, car_restrictions: list[dict[str, Any]], fixed_setup: bool
    ) -> None:
            ...
            try:
                ...
                CarRestriction.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        "power_adjust_pct": restriction_data.get(
                            "power_adjust_pct", 0.0
                        ),
        ...
        for car_class_id in car_class_ids:
            try:
                ...
                car_class = CarClass.objects.get(
                    sim_api_id=car_class_id, simulator=self.simulator
                )
    ...
    def _process_time_patterns(
        self, event: Event, race_time_descriptors: list[dict[str, Any]]
    ) -> None:
        """
    ...
    def _create_specific_time_slots(
        self, event: Event, time_descriptor: dict[str, Any]
    ) -> None:
        ...
        if not session_times:
            logger.warning(
                f"No session_times found in specific time pattern for event {event.name}"
            )
            return
            ...
            try:
                ...
                if isinstance(session_time_str, str):
                    session_time = datetime.fromisoformat(
                        session_time_str.replace("Z", "+00:00")
                    )
                else:
                ...
                registration_open = session_time - timedelta(
                    minutes=event.registration_open_minutes
                )
                ...
                if created:
                    ...
                    logger.debug(
                        f"Created specific time slot for {event.name} at {session_time}"
                    )
                else:
                    logger.debug(
                        f"Time slot already exists for {event.name} at {session_time}"
                    )

            except (ValueError, TypeError) as e:
                logger.warning(
                    f"Error parsing session time '{session_time_str}' for event {event.name}: {e}"
                )
                continue
...
class RecurrenceHandler:
    ...
    def _generate_repeating_slots(
            ...
            if isinstance(pattern_start_date, str):
                ...
                if "T" in pattern_start_date or "Z" in pattern_start_date:
                    base_date = datetime.fromisoformat(
                        pattern_start_date.replace("Z", "+00:00")
                    )
                else:
        ...
        logger.debug(
            f"Generating repeating slots from {current_date} to {end_date_only} for event {event.name}"
        )
        logger.debug(
            f"Pattern: every {repeat_minutes}min starting at {first_session_time} on days {day_offsets}"
        )
        ...
        while current_date <= end_date_only:
            ...
            if day_offset in day_offsets:
                ...
                logger.debug(
                    f"Generated {slot_count} slots for {current_date} (day_offset {day_offset})"
                )
            ...
            try:
                if end_str:
                    if isinstance(end_str, str):
                        ...
                        end_date = (
                            end_str.date() if hasattr(end_str, "date") else end_str
                        )
