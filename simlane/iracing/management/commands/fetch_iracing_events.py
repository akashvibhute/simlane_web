        ...
        try:
            if use_async:
                ...
                try:
                    ...
                    self.stdout.write(f"Fetched race guide with {len(race_guide_data.get('sessions', []))} sessions")
                ...
                try:
                    ...
                    self.stdout.write(f"Fetched {len(series_seasons_data)} series seasons")
                ...
                if result.get("success"):
                    ...
                    raise CommandError(f"Task failed: {result.get('error', 'Unknown error')}")

        except Exception as e:
            logger.exception("Error executing fetch_iracing_events command")
            raise CommandError(f"Failed to fetch iRacing events: {str(e)}")
