...
class Command(BaseCommand):
    ...
    def process_iracing_cars(
        self, simulator: Simulator, cars_data: Any, force_update: bool
    ):
        ...
        for track_name, track_configs in tracks_by_name.items():
            try:
                ...
                if track_created:
                    ...
                    sim_track.is_laser_scanned = any(
                        config.get("tech_track", False) for config in track_configs
                    )
                ...
                for config in track_configs:
                    ...
                    if layout_created:
                        ...
                        layout.length_km = (
                            config.get("track_config_length", 0) / 1000
                            if config.get("track_config_length")
                            else 0
                        )
                        layout.save()
