...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(f"\nCATEGORIES:")
            self.stdout.write(f"  - Cars with multiple categories: {len(cars_with_multiple_categories)}")
            self.stdout.write(f"  - Cars with no categories: {len(cars_with_no_categories)}")
            self.stdout.write(f"  - Unique categories found: {list(categories_counter.keys())}")
            ...
            self.stdout.write(f"\nCAR_TYPES:")
            self.stdout.write(f"  - Cars with multiple types: {len(cars_with_multiple_types)}")
            self.stdout.write(f"  - Cars with no types: {len(cars_with_no_types)}")
            self.stdout.write(f"  - Unique car types found: {list(car_types_counter.keys())}")
