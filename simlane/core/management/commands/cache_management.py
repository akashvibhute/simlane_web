...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options['clear']:
            ...
            self.stdout.write(
                self.style.ERROR('Please specify an action: --clear, --warm, --stats, or --test')
            )
    ...
    def warm_caches(self):
        ...
        try:
            ...
            popular_profiles = SimProfile.objects.filter(
                is_public=True
            ).select_related('simulator').order_by('-created_at')[:20]
            ...
            active_clubs = Club.objects.filter(is_active=True).order_by('-created_at')[:10]
            clubs_warmed = 0
            ...
            for alias in cache_aliases:
                try:
                    ...
                    try:
                        ...
                        self.stdout.write(f"  Redis stats unavailable: {str(e)[:50]}...")
