...
from simlane.sim.models import (
    Simulator, CarClass, CarModel, SimCar, TrackModel, SimTrack, SimLayout,
    PitData, Series, Event, EventClass, EventInstance, WeatherForecast,
    SimProfile, FuelUnit, TrackType, EventType, EventStatus, SessionType,
    RacingDiscipline
)
from simlane.teams.models import (
    Club, ClubMember, Team, TeamMember, ClubEvent, EventSignup, ClubRole
)
...
class Command(BaseCommand):
    ...
    def clear_data(self):
        ...
        models_to_clear = [
            WeatherForecast, EventSignup, ClubEvent, TeamMember, Team,
            ClubMember, Club, EventInstance, EventClass, Event, Series,
            SimLayout, SimTrack, TrackModel, SimCar, CarModel, CarClass,
            SimProfile, Simulator, PitData
        ]
    ...
    def create_users(self):
        """Create test users"""
        users_data = [
            {'username': 'admin_user', 'email': 'admin@example.com', 'name': 'Admin User', 'is_staff': True, 'is_superuser': True},
            {'username': 'john_doe', 'email': 'john@example.com', 'name': 'John Doe'},
            {'username': 'jane_smith', 'email': 'jane@example.com', 'name': 'Jane Smith'},
            {'username': 'mike_wilson', 'email': 'mike@example.com', 'name': 'Mike Wilson'},
            {'username': 'sarah_jones', 'email': 'sarah@example.com', 'name': 'Sarah Jones'},
            {'username': 'alex_brown', 'email': 'alex@example.com', 'name': 'Alex Brown'},
            {'username': 'emma_davis', 'email': 'emma@example.com', 'name': 'Emma Davis'},
            {'username': 'chris_miller', 'email': 'chris@example.com', 'name': 'Chris Miller'},
        ]
    ...
    def create_car_classes(self):
        """Create car classes"""
        classes_data = [
            {'name': 'GT3', 'category': 'Sports Car', 'description': 'GT3 class sports cars'},
            {'name': 'GTE/GTLM', 'category': 'Sports Car', 'description': 'GTE/GTLM class sports cars'},
            {'name': 'LMP2', 'category': 'Prototype', 'description': 'LMP2 prototype cars'},
            {'name': 'Formula 3', 'category': 'Formula', 'description': 'Formula 3 single seaters'},
            {'name': 'NASCAR Cup', 'category': 'Stock Car', 'description': 'NASCAR Cup Series cars'},
            {'name': 'Touring Car', 'category': 'Touring', 'description': 'Touring car category'},
        ]
    ...
    def create_car_models(self, car_classes):
        ...
        pit_data_configs = [
            {'drive_through_loss_sec': 25.0, 'stop_go_base_loss_sec': 30.0, 'fuel_unit': FuelUnit.LITER, 'refuel_flow_rate': 2.5, 'tire_change_all_four_sec': 12.0},
            {'drive_through_loss_sec': 28.0, 'stop_go_base_loss_sec': 35.0, 'fuel_unit': FuelUnit.LITER, 'refuel_flow_rate': 3.0, 'tire_change_all_four_sec': 15.0},
            {'drive_through_loss_sec': 22.0, 'stop_go_base_loss_sec': 28.0, 'fuel_unit': FuelUnit.LITER, 'refuel_flow_rate': 4.0, 'tire_change_all_four_sec': 10.0},
        ]
        ...
        cars_data = [
            # GT3 Cars
            {'manufacturer': 'BMW', 'name': 'M4 GT3', 'car_class': 'GT3', 'release_year': 2022},
            {'manufacturer': 'Mercedes-AMG', 'name': 'GT3 2020', 'car_class': 'GT3', 'release_year': 2020},
            {'manufacturer': 'Porsche', 'name': '911 GT3 R', 'car_class': 'GT3', 'release_year': 2019},
            {'manufacturer': 'Ferrari', 'name': '488 GT3 Evo 2020', 'car_class': 'GT3', 'release_year': 2020},
            
            # GTE Cars
            {'manufacturer': 'Ford', 'name': 'GT GTE', 'car_class': 'GTE/GTLM', 'release_year': 2018},
            {'manufacturer': 'Corvette', 'name': 'C8.R GTE', 'car_class': 'GTE/GTLM', 'release_year': 2020},
            
            # LMP2
            {'manufacturer': 'Dallara', 'name': 'P217 LMP2', 'car_class': 'LMP2', 'release_year': 2017},
            
            # Formula
            {'manufacturer': 'Dallara', 'name': 'F3 2019', 'car_class': 'Formula 3', 'release_year': 2019},
            
            # NASCAR
            {'manufacturer': 'Chevrolet', 'name': 'Camaro ZL1', 'car_class': 'NASCAR Cup', 'release_year': 2022},
            {'manufacturer': 'Ford', 'name': 'Mustang GT', 'car_class': 'NASCAR Cup', 'release_year': 2022},
        ]
    ...
    def create_track_models(self):
        ...
        tracks_data = [
            ...
            {'name': 'Circuit de la Sarthe', 'country': 'France', 'location': 'Le Mans'},
            {'name': 'Nürburgring', 'country': 'Germany', 'location': 'Nürburg'},
            {'name': 'Silverstone Circuit', 'country': 'United Kingdom', 'location': 'Silverstone'},
            {'name': 'Monza', 'country': 'Italy', 'location': 'Monza'},
            {'name': 'Suzuka Circuit', 'country': 'Japan', 'location': 'Suzuka'},
            {'name': 'Daytona International Speedway', 'country': 'USA', 'location': 'Daytona Beach, FL'},
    ...
    def create_sim_layouts(self, sim_tracks):
        ...
        layouts_data = [
            {'layout_code': 'FULL', 'name': 'Full Course', 'type': TrackType.ROAD, 'length_km': 7.004},
            {'layout_code': 'FULL', 'name': 'Full Circuit', 'type': TrackType.ROAD, 'length_km': 13.626},
            {'layout_code': 'GP', 'name': 'Grand Prix', 'type': TrackType.ROAD, 'length_km': 5.148},
            {'layout_code': 'FULL', 'name': 'Full Circuit', 'type': TrackType.ROAD, 'length_km': 5.891},
            {'layout_code': 'GP', 'name': 'Grand Prix', 'type': TrackType.ROAD, 'length_km': 5.793},
            {'layout_code': 'GP', 'name': 'Grand Prix', 'type': TrackType.ROAD, 'length_km': 5.807},
            {'layout_code': 'OVAL', 'name': 'Oval', 'type': TrackType.OVAL, 'length_km': 4.023},
            {'layout_code': 'FULL', 'name': 'Full Course', 'type': TrackType.ROAD, 'length_km': 6.515},
        ]
        
        sim_layouts = []
        for sim_track, layout_data in zip(sim_tracks, layouts_data):
    ...
    def create_series(self):
        ...
        series_data = [
            {'name': 'IMSA SportsCar Championship', 'description': 'Premier sports car racing series', 'is_team_event': True, 'min_drivers_per_entry': 2, 'max_drivers_per_entry': 4},
            {'name': 'Blancpain GT Series', 'description': 'GT3 championship series', 'is_team_event': True, 'min_drivers_per_entry': 2, 'max_drivers_per_entry': 3},
            {'name': 'Formula Renault 2.0', 'description': 'Single seater championship', 'is_team_event': False, 'min_drivers_per_entry': 1, 'max_drivers_per_entry': 1},
            {'name': 'Touring Car Championship', 'description': 'Touring car racing series', 'is_team_event': False, 'min_drivers_per_entry': 1, 'max_drivers_per_entry': 2},
        ]
    ...
    def create_events(self, simulator, sim_layouts, series):
        ...
        events = []
        event_names = [
            'Spa 6 Hours', 'Le Mans 24 Hours', 'Nürburgring 4 Hours',
            'Silverstone 6 Hours', 'Monza Sprint Race', 'Suzuka 8 Hours',
            'Daytona 24 Hours', 'Road America 500'
        ]
        
        for i, (name, sim_layout, series_obj) in enumerate(zip(event_names, sim_layouts, series)):
            ...
            event, created = Event.objects.get_or_create(
                ...
                defaults={
                    ...
                    'weather': {'temperature': random.randint(15, 30), 'conditions': 'Clear'},
                }
            )
            events.append(event)
    ...
    def create_event_classes(self, events, car_classes, sim_cars):
        ...
        for event in events:
            ...
            available_classes = random.sample(car_classes, min(num_classes, len(car_classes)))
            
            for car_class in available_classes:
                # Get cars for this class
                class_cars = [car for car in sim_cars if car.car_model.car_class == car_class]
                if class_cars:
                    num_cars = min(len(class_cars), random.randint(1, max(1, len(class_cars))))
    ...
    def create_event_instances(self, events):
        ...
        for event in events:
            ...
            for i, instance_data in enumerate(instances_data):
                start_time = base_time + timedelta(minutes=instance_data['start_offset'])
    ...
    def create_clubs(self, users):
        """Create racing clubs"""
        clubs_data = [
            {'name': 'Apex Racing Club', 'description': 'Premier endurance racing club', 'is_public': True},
            {'name': 'SimSpeed Racing', 'description': 'Competitive GT racing community', 'is_public': True},
            {'name': 'Velocity Motorsports', 'description': 'Multi-class racing club', 'is_public': False},
            {'name': 'Thunder Racing League', 'description': 'Oval and road racing specialists', 'is_public': True},
        ]
        ...
        for i, club_data in enumerate(clubs_data):
            ...
            for user in club_users:
                role = ClubRole.ADMIN if user == creator else random.choice([ClubRole.MEMBER, ClubRole.TEAMS_MANAGER])
    ...
    def create_teams(self, clubs):
        ...
        for club in clubs:
            ...
            for i in range(num_teams):
                ...
                team_members = random.sample(club_members, min(team_size, len(club_members)))
    ...
    def create_club_events(self, clubs, events, users):
        ...
        for club in clubs:
            ...
            for event in selected_events:
                creator = random.choice(list(club.members.filter(role__in=[ClubRole.ADMIN, ClubRole.TEAMS_MANAGER])))
                signup_deadline = event.event_date - timedelta(days=random.randint(1, 7))
    ...
    def create_event_signups(self, club_events, users, sim_cars, teams):
        ...
        for club_event in club_events:
            ...
            signup_count = random.randint(min(6, max_signups), max_signups) if max_signups > 0 else 0
            selected_members = random.sample(list(club_members), signup_count) if signup_count > 0 else []
            ...
            for member in selected_members:
                ...
                assigned_team = random.choice(club_teams) if club_teams and random.choice([True, False]) else None
