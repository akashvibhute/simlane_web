    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if isinstance(cars_data, dict):
                ...
                self.stdout.write(
                    f"Cars data is a dict with keys: {list(cars_data.keys())}"
                )
            else:
            ...
            for i, car in enumerate(cars[:10]):  # Look at first 10 cars
                ...
                example = {
                    ...
                    "car_make": car_make,
                    "car_model": car_model,
                    "suggested_manufacturer": car_make
                    if car_make
                    else self._extract_manufacturer(car_name),
                    "suggested_model": car_model
                    if car_model
                    else self._extract_model(car_name, car_make),
                }
                ...
                self.stdout.write(
                    f"    → Suggested manufacturer: '{example['suggested_manufacturer']}'"
                )
                self.stdout.write(
                    f"    → Suggested model: '{example['suggested_model']}'"
                )
                self.stdout.write("")
            ...
            self.stdout.write(
                "  - name: Use car_model if available, else extract from car_name"
            )
            self.stdout.write(
                "  - manufacturer: Use car_make if available, else extract from car_name"
            )
