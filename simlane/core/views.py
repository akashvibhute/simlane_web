...
def dashboard_view(request):
    ...
    try:
        ...
        club_memberships = ClubMember.objects.filter(user=request.user).select_related(
            "club"
        )
...
def search_htmx(request):
    ...
    if not query:
        return render(
            request,
            "core/search/search_results_partial.html",
            {
                "results": [],
                "total_count": 0,
                "query": "",
    ...
    context = {
        ...
        "query": query,
        "query_time_ms": search_results["query_time_ms"],
        "show_more_url": f"/search/?q={query}"
        if search_results["total_count"] > 8
        else None,
    }
