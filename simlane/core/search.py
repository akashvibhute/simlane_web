...
class SearchService(ABC):
    ...
    @abstractmethod
    def search_by_type(
        self, query: str, model_type: str, limit: int = 20
    ) -> list[SearchResult]:
...
class SearchDocumentBuilder:
    ...
    def _build_simprofile_document(profile) -> SearchResult:
        return SearchResult(
            ...
            + (
                f" - Linked to {profile.linked_user.username}"
            ...
            metadata={
                ...
                "linked_user": profile.linked_user.username
                if profile.linked_user
                else None,
    ...
    def _build_team_document(team) -> SearchResult:
        return SearchResult(
            ...
            type="team",
            title=team.name,
            description=team.description[:200] + "..."
            if len(team.description) > 200
            ...
            metadata={
                ...
                "source_simulator": team.source_simulator.name
                if team.source_simulator
                else None,
            },
        )
    ...
    def _build_simulator_document(simulator) -> SearchResult:
        return SearchResult(
            ...
            description=simulator.description[:200] + "..."
            if len(simulator.description) > 200
            else simulator.description,
...
class PostgresSearchService(SearchService):
    ...
    def search(
        ...
        search_models = (
            filters.types if filters.types else list(self.searchable_models.keys())
        )
        ...
        try:
            ...
            for instance in queryset:
                try:
                    ...
                    doc.relevance_score = (
                        float(instance.rank) if hasattr(instance, "rank") else 0.0
                    )
                    results.append(doc)
                except Exception as e:
        ...
        configs = {
            ...
            "event": {
                "fields": [
                    "name",
                    "description",
                    "sim_layout__sim_track__display_name",
                ],
            },
            "team": {
    ...
    def _apply_model_filters(self, queryset, model_type: str):
        ...
        elif model_type == "club" or model_type == "simulator":
    ...
    def _build_facets(self, results: list[SearchResult]) -> dict[str, Any]:
        ...
        for result in results:
            if (
                result.type in ["sim_profile", "event"]
                and "simulator" in result.metadata
            ):
