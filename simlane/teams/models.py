...
class Team(models.Model):
    ...
    class Meta:
        unique_together = [
            ...
            [
                "owner_sim_profile",
                "slug",
            ],  # Unique slug per sim profile (when imported)
    ...
    def effective_owner(self):
        ...
        if self.owner_sim_profile and self.owner_sim_profile.user:
    ...
    def is_claimable(self):
        ...
        return (
            self.is_imported
            and self.owner_sim_profile
            and not self.owner_sim_profile.user
        )
    ...
    def can_user_manage(self, user):
        ...
        if self.owner_sim_profile and self.owner_sim_profile.user == user:
            return True
...
class EventParticipation(models.Model):
    ...
    assigned_time_slot = models.ForeignKey(
        ...
        blank=True,
        related_name="assigned_participations",
        help_text="Final time slot assignment after team formation",
    )
    ...
    class Meta:
        constraints = [
            ...
            # User can only participate in one time slot at a time
    ...
    def can_be_assigned_to_team(self):
        ...
        return (
            self.status in ["signed_up", "team_formation"]
            and self.participation_type == "team_signup"
        )
...
class AvailabilityWindow(models.Model):
    ...
    def __str__(self):
        ...
        if self.can_spot:
            roles.append("spot")
        if self.can_strategize:
            roles.append("strategize")
        return (
            f"{participant} - {self.start_time} to {self.end_time} ({'/'.join(roles)})"
        )
    ...
    def overlaps_with(self, other_window):
        ...
        return (
            self.start_time < other_window.end_time
            and self.end_time > other_window.start_time
        )
    ...
    def find_available_for_time_range(cls, event, start_time, end_time, role="drive"):
        ...
        """
        role_field = (
            f"can_{role}" if role in ["drive", "spot", "strategize"] else "can_drive"
        )
            ...
            chart_data.append(
                {
                    ...
                    "user_id": window.participation.user.id
                    if window.participation.user
                    else None,
...
class RaceStrategy(models.Model):
    ...
    time_slot = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        related_name="race_strategies",
        help_text="Specific time slot/session this strategy is for",
    )
    ...
    def calculate_total_stints(self):
        ...
        if not self.time_slot.session:
            return 0

        session_duration = self.time_slot.session.duration  # in minutes
...
class ClubEventSignupSheet(models.Model):
    ...
    def is_open(self):
        ...
        now = timezone.now()
        return self.status == "open" and self.signup_opens <= now <= self.signup_closes
    ...
    def can_open_signups(self):
        ...
        return self.status == "draft" and self.signup_opens <= timezone.now()
    ...
    def get_signups(self):
        ...
        return (
            self.event.participations.filter(
                signup_context_club=self.club,
            )
            .select_related("user")
            .prefetch_related("availability_windows")
