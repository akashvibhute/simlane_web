...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        from simlane.teams.services import AvailabilityService
        from simlane.teams.services import EventParticipationService
        ...
        for i, user in enumerate(users):
            ...
            self.create_availability_windows(
                participation, event_start, user_timezone, AvailabilityService
            )

        self.stdout.write(
            f"Created {len(participations)} participations with availability"
        )

        # Show summary
        self.show_summary(event, club_event, participations)

    def create_availability_windows(
        self, participation, event_start, timezone_str, AvailabilityService
    ):
        ...
        if pattern == "full":
            ...
            windows.append(
                {
                    "start_time_local": event_start
                    + timedelta(hours=random.randint(4, 16)),
                    "end_time_local": event_start
                    + timedelta(hours=random.randint(18, 22)),
                    "can_drive": True,
                    "can_spot": True,
    ...
    def show_summary(self, event, club_event, participations):
        ...
        from simlane.teams.services import AvailabilityService
        from simlane.teams.services import EventParticipationService
        from simlane.teams.services import WorkflowService
        ...
        # Get coverage report
        coverage = AvailabilityService.generate_coverage_report(
            event, timezone_display="UTC"
        )
        ...
        covered_hours = sum(
            1 for h in coverage["hourly_coverage"].values() if h["drivers"] > 0
        )
        self.stdout.write(
            f"Coverage: {covered_hours}/{total_hours} hours ({covered_hours / total_hours * 100:.0f}%)"
        )
