...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if verbose:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Looking for invitations expired before: {cutoff_date}",
                ),
            )
        ...
        self.stdout.write(
            self.style.WARNING(
                f"Found {len(expired_invitations)} expired invitations:",
            ),
        )
        ...
        if not options.get("force", False):
            confirm = input(
                f"Delete {len(expired_invitations)} expired invitations? [y/N]: ",
            )
    ...
    def _get_statistics(self):
        ...
        stats = {
            ...
            "pending_invitations": ClubInvitation.objects.filter(
                status="PENDING",
            ).count(),
            "expired_invitations": ClubInvitation.objects.filter(
                status="EXPIRED",
            ).count(),
            "accepted_invitations": ClubInvitation.objects.filter(
                status="ACCEPTED",
            ).count(),
            "declined_invitations": ClubInvitation.objects.filter(
                status="DECLINED",
            ).count(),
        }
