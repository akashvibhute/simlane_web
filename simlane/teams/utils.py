...
def export_signup_data(club_event, format: str = "csv") -> io.BytesIO:
    ...
    if format == "csv":
        ...
        for signup in club_event.signups.all():
            ...
            availability_pct = (
                (available_instances / total_instances * 100)
                if total_instances > 0
                else 0
            )
...
class AvailabilityChartGenerator:
    ...
    @staticmethod
    def generate_coverage_heatmap_data(event, timezone_display='UTC', resolution_hours=1) -> Dict[str, Any]:
        ...
        ).values('id', 'username', 'first_name', 'last_name').distinct())
    ...
    @staticmethod
    def generate_team_overlap_chart(user_ids: List[int], event, timezone_display='UTC') -> Dict[str, Any]:
...
class TeamFormationAlgorithms:
    ...
    @staticmethod
    def greedy_team_formation(event, team_size: int = 3, max_teams: int = None) -> List[List[int]]:
        """
        ...
        participants = list(EventParticipation.objects.filter(
            event=event,
            status='signed_up',
        ...
        all_overlaps = AvailabilityWindow.find_overlapping_availability(participants, event)
    ...
    def balanced_team_formation(event, team_size: int = 3) -> List[Dict[str, Any]]:
        ...
        while len(remaining_users) >= team_size:
            ...
            for team_combo in combinations(remaining_users, team_size):
                ...
                team_overlaps = [o for o in overlaps 
                               if o['user1_id'] in team_list and o['user2_id'] in team_list]
                avg_overlap = sum(o['total_overlap_hours'] for o in team_overlaps) / max(len(team_overlaps), 1)
                
                # Combined score (lower variance = better balance, higher overlap = better)
                balance_score = avg_overlap / max(exp_variance, 0.1)  # Avoid division by zero
            ...
            if best_team:
                team_info = {
                    ...
                    'average_experience': sum(user_data[uid]['experience'] for uid in best_team) / len(best_team),
                    'total_overlap_hours': sum(
                        o['total_overlap_hours'] for o in overlaps 
...
class AvailabilityConflictDetector:
    ...
    @staticmethod
    def detect_stint_conflicts(team_members: List[int], proposed_stints: List[Dict]) -> List[Dict]:
        """
            ...
            if not available_windows.exists():
                ...
                if partial_windows.exists():
                    conflicts.append({
                        ...
                        'stint_end': end_time,
                        'available_windows': list(partial_windows.values(
    ...
    @staticmethod
    def suggest_alternative_stints(team_members: List[int], event, 
                                 total_duration_hours: int) -> List[Dict]:
        """
        ...
        for period in sorted(continuous_periods, key=lambda x: x['start']):
            ...
            if period['start'] <= last_period['end']:
                ...
                all_drivers = set(last_period['available_drivers'] + period['available_drivers'])
