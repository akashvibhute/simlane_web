...
class EventParticipationService:
    ...
    def get_participation_summary(event: Event) -> dict[str, Any]:
        ...
        for participation in participations:
            summary["by_type"][participation.participation_type] = (
                summary["by_type"].get(participation.participation_type, 0) + 1
            )
            summary["by_status"][participation.status] = (
                summary["by_status"].get(participation.status, 0) + 1
            )
...
class AvailabilityService:
    ...
    def generate_coverage_report(
        ...
        for hour_data in coverage.values():
            ...
            hour_data["coverage_percentage"] = (
                (hour_data["total_available"] / total_participants * 100)
...
class TeamFormationService:
    ...
    def analyze_compatibility(event: Event, user_ids: list[int]) -> dict[str, Any]:
        ...
        for overlap in overlaps:
            ...
            total_overlap_by_user[user1] = (
                total_overlap_by_user.get(user1, 0) + overlap_hours
            )
            total_overlap_by_user[user2] = (
                total_overlap_by_user.get(user2, 0) + overlap_hours
            )
            ...
            enhanced_recommendations.append(
                {
                    ...
                    "recommended_car": max(car_preferences.keys())
                    if car_preferences
                    else None,
    ...
    @staticmethod
    def create_teams_from_recommendations(
        event: Event,
