...
from .models import (
    Club, ClubMember, ClubRole, ClubInvitation,
    ClubEvent, EventSignup, EventSignupAvailability,
    Team, TeamAllocation, TeamAllocationMember,
    TeamEventStrategy, StintAssignment,
    EventEntry, PredictedStint
)
...
class ClubInvitationService:
    ...
    @staticmethod
    def send_invitation(club: Club, inviter: User, email: str, role: str, message: str = "") -> ClubInvitation:
        ...
        try:
            ...
            html_message = render_to_string('templates/emails/club_invitation.html', context)
    ...
    def accept_invitation(token: str, user: User) -> ClubMember:
        ...
        try:
            ...
            logger.info(f"User {user.username} accepted invitation to {invitation.club.name}")
            return club_member
...
class EventSignupService:
    ...
    @staticmethod
    @transaction.atomic
    def create_signup_sheet(club: Club, event, creator: User, details: dict) -> ClubEvent:
    ...
    def _send_signup_confirmation(signup: EventSignup) -> None:
        ...
        html_message = render_to_string('emails/event_signup_confirmation.html', context)
        text_message = strip_tags(html_message)
...
class TeamAllocationService:
    ...
    def suggest_team_allocations(
        ...
        for signup in signups:
            ...
            signup_data.append({
                ...
                'lap_count': lap_count,
                'availability_score': TeamAllocationService._calculate_availability_score(signup),
                'car_preference_match': TeamAllocationService._calculate_car_preference_match(signup, club_event)
            })
    ...
    def validate_allocation(allocation_data: Dict) -> Tuple[bool, List[str]]:
        ...
        if club_event:
            if member_count < club_event.team_size_min:
                errors.append(f"Team must have at least {club_event.team_size_min} members")
        ...
        drivers = sum(1 for m in allocation_data.get('members', []) 
                     if m['signup'].can_drive)
        if drivers == 0:
    ...
    @staticmethod
    def _calculate_car_preference_match(signup: EventSignup, club_event: ClubEvent) -> float:
    ...
    def _notify_team_assignments(club_event: ClubEvent) -> None:
        ...
        for signup in signups:
            ...
            html_message = render_to_string('emails/team_allocation_notification.html', context)
            text_message = strip_tags(html_message)
...
class StintPlanningService:
    ...
    def generate_stint_plan(
        ...
        avg_stint_length = min(120, max(30, total_minutes // (driver_count * 2)))  # 2 stints per driver
        ...
        while current_time < event_instance.end_time:
            ...
            stint = StintAssignment(
                ...
                estimated_duration_minutes=int((stint_end - current_time).total_seconds() / 60),
                role='primary_driver'
            )
            stints.append(stint)
    ...
    @staticmethod
    def _calculate_pit_duration(pit_data, fuel_amount: float, tire_change: bool) -> float:
