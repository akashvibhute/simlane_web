...
class ClubEventSignupSheetForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.club:
            ...
            existing_event_ids = self.club.event_signup_sheets.values_list(
                "event_id", flat=True
            )
            ...
            self.fields["event"].queryset = (
                ...
                .filter(
                    # Only future events or events with time slots in the future
                    time_slots__start_time__gt=timezone.now(),
                )
                .distinct()
                .order_by("time_slots__start_time")
            )

        # Convert datetime fields to local timezone for display
        if self.instance and self.instance.pk:
            if self.instance.signup_opens:
                self.initial["signup_opens"] = self.instance.signup_opens.strftime(
                    "%Y-%m-%dT%H:%M"
                )
            if self.instance.signup_closes:
                self.initial["signup_closes"] = self.instance.signup_closes.strftime(
                    "%Y-%m-%dT%H:%M"
                )
    ...
    def clean(self):
        ...
        if signup_opens and signup_closes:
            ...
            event = cleaned_data.get("event")
            if event:
                earliest_time_slot = event.time_slots.order_by("start_time").first()
                if earliest_time_slot and signup_closes > earliest_time_slot.start_time:
                    raise ValidationError(
                        f"Signups must close before the event starts ({earliest_time_slot.start_time})",
                    )
        ...
        if min_drivers and max_drivers and min_drivers > max_drivers:
            raise ValidationError(
                "Minimum drivers cannot be greater than maximum drivers"
            )

        if target_size:
            if min_drivers and target_size < min_drivers:
                raise ValidationError(
                    "Target team size cannot be less than minimum drivers"
                )
            if max_drivers and target_size > max_drivers:
                raise ValidationError(
                    "Target team size cannot be greater than maximum drivers"
                )
    ...
    def clean(self):
        ...
        if timing == "fixed":
            ...
            if not opens or not closes:
                raise ValidationError(
                    "Fixed open and close times are required when using fixed timing"
                )
    ...
    def create_signup_sheets(self):
        ...
        for event in events:
            ...
            if timing == "immediate":
                ...
                earliest_time_slot = event.time_slots.order_by("start_time").first()
                if earliest_time_slot:
                    signup_closes = earliest_time_slot.start_time - timedelta(days=2)
                else:
                ...
                earliest_time_slot = event.time_slots.order_by("start_time").first()
                if earliest_time_slot:
                    signup_opens = earliest_time_slot.start_time - timedelta(
                        days=self.cleaned_data["days_before_open"],
                    )
                    signup_closes = earliest_time_slot.start_time - timedelta(
            ...
            title = title_template.format(
                ...
                date=earliest_time_slot.start_time.strftime("%b %d")
                if earliest_time_slot
                else "TBD",
            )
