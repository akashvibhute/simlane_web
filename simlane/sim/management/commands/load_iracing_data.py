...
class Command(BaseCommand):
    ...
    def process_tracks(self, simulator: Simulator, tracks_data: Any, force_update: bool):
        ...
        for track_name, track_configs in tracks_by_name.items():
            try:
                ...
                # Prepare image URLs for downloading
                track_folder = base_track.get('folder', '')  # Get the folder path from API
                image_urls = {
                ...
                if not track_created and force_update:
                    ...
                    for field, value in sim_track_defaults.items():
                        if field not in ['sim_api_id', 'slug']:  # Don't update these IDs
                    ...
                    for field, url in image_urls.items():
                        ...
                        if url and self._should_update_image(current_value, url):
                            image_file = self._download_and_save_image(url, field, sim_track.display_name, track_folder)
                            if image_file:
    ...
    def _download_and_save_image(self, image_url: str, field_name: str, car_name: str, folder: str | None = None) -> ContentFile | None:
        """
        ...
        try:
            ...
            if image_url.startswith('/'):
                ...
                if folder:
                    ...
                    if '-small.' in image_url or '-large.' in image_url:
                        car_folder = image_url.rsplit('-', 1)[0]  # Remove -small/-large part
            ...
            self.stdout.write(
                self.style.WARNING(f"    Failed to download {field_name} from {image_url}: {e}")
            )
            return None
        except Exception as e:
            self.stdout.write(
