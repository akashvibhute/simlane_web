...
def download_image_from_url(url: str, timeout: int = 30) -> ContentFile | None:
    """
    ...
    try:
        ...
        if not content_type.startswith("image/"):
            logger.warning(
                f"URL does not return an image: {url} (content-type: {content_type})"
            )
            return None
...
def save_car_images(car_model, car_data: dict) -> dict:
    ...
    if screenshot_images:
        screenshot_list = [
            img.strip() for img in screenshot_images.split(",") if img.strip()
        ]
        ...
        for i, screenshot_name in enumerate(screenshot_list):
            try:
                ...
                if not screenshot_name.startswith("http"):
                    screenshot_url = (
                        f"{IRACING_BASE_URL}/img/cars/{screenshot_name}.jpg"
                    )
                else:
                ...
                logger.error(
                    f"Error saving screenshot {screenshot_name} for {car_model.full_name}: {e}"
                )
                results["gallery_errors"] += 1
...
def save_track_images(sim_track, track_data: dict) -> dict:
    ...
    if logo_url:
        if _should_update_track_image(sim_track.logo, logo_url):
            ...
            results["skipped"] += 1
            logger.info(
                f"Skipped logo for {sim_track.display_name} - file already exists"
            )
    ...
    if small_image_url:
        if _should_update_track_image(sim_track.small_image, small_image_url):
            ...
            results["skipped"] += 1
            logger.info(
                f"Skipped small image for {sim_track.display_name} - file already exists"
            )
    ...
    if large_image_url:
        if _should_update_track_image(sim_track.large_image, large_image_url):
            ...
            results["skipped"] += 1
            logger.info(
                f"Skipped large image for {sim_track.display_name} - file already exists"
            )
...
def save_track_svg_maps(sim_layout, track_data: dict) -> dict:
    ...
    if existing_svg_maps.exists():
        ...
        expected_captions = set(
            layer_name.replace("-", " ").title() for layer_name in layers_data.keys()
        )
        ...
        try:
            ...
            if svg_file:
                ...
                MediaGallery.objects.create(
                    ...
                    image=svg_file,
                    caption=layer_name.replace(
                        "-", " "
                    ).title(),  # "start-finish" -> "Start Finish"
                    order=i,
